...
pub async fn revisar_profesor(
    ctx: Context<'_>,
    #[description = "Periodo a seleccionar"] periodo: Periodos,
    #[description = "Profesor del curso"] profesor: String,
) -> Result<(), Error> {
    let profesor_string: String = profesor.replace(" ", "+");
    let periodo_string: String = periodo.into();
    let cursos_busqueda = buscar_profesor(&periodo_string, &profesor_string).await;
